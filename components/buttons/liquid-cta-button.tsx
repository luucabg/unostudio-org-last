...
function ButtonContent({ children, theme }: Pick<LiquidCtaButtonProps, "children" | "theme">) {
...
export function LiquidCtaButton({ children, className, onClick, theme = "dark", href }: LiquidCtaButtonProps) {
  const rootClassName = cn(
    "group inline-block transition-transform duration-300 hover:scale-105 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300 rounded-full",
    className,
  )
  ...
  if (href) {
    return (
      <a href={href} className={rootClassName}>
        <ButtonContent theme={theme}>{children}</ButtonContent>
      </a>
    )
  }
